...
local number_of_lines = tonumber(arg[1])
local file = io.open("ex22_4.lua", "r")
local lines = {}
function seekChunksAndPopulate(n)
    file:seek("end", -512 * n)
    local i = 0
    while true do
...
local i = 1
while true do
    seekChunksAndPopulate(i)
    if #lines < number_of_lines then
        lines = {}
        i = i + 1
    else
        break;
...
for i = #lines - number_of_lines + 1, #lines do
    io.write(lines[i])
end
